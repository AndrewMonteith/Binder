...
from recommender.pandas_helpers import create_meand_df, update_meand_df
...
def add_user_rating(user_id, book_id, rating):
    ratings.loc[len(ratings) + 1] = [user_id, book_id, rating]
    new_user_mean = update_meand_df(book_mean_dataframe, book_id, rating)
    new_book_mean = update_meand_df(user_mean_dataframe, user_id, rating)
